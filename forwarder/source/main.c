 ...
 * wo1wan forwarder
 *
 * This is a Nintendo Switch APPLICATION (not a homebrew applet). It loads
 * sdmc:/switch/wo1wan.nro and runs it as AppletType_SystemApplication, which
 * is the ONLY context that is allowed to launch the Web Applet (the Switch
 * built-in browser). This is exactly how wiliwili's desktop icon works.
 *
 * The Web Applet cannot be launched from inside hbmenu / a LibraryApplet,
 * which is why a plain .nro from hbmenu always shows "Unable to use this
 * feature". Running as an Application fixes that.
 *
 * Forked from wiliwili/scripts/switch-forwarder/source/main.c (nx-hbloader).
 */
...
#define DEFAULT_NRO   "sdmc:/switch/wo1wan.nro"
#define APP_STORE_NRO "sdmc:/switch/wo1wan/wo1wan.nro"

const char g_noticeText[] =
    "wo1wan forwarder " VERSION
    "\0"
    "wo1wan - play web games on Switch (Application forwarder)";
...
#define M EntryFlag_IsMandatory
    ...
    entries[11].Value[1] =
        hosversionIsAtmosphere() ? 0x41544d4f53504852UL : 0;
